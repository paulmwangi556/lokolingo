	...
	if request.user.is_superuser or request.user.is_staff:
		if request.method == 'POST':
			...
			while i <= size_no:
				...
				i+=1
			Product(product_id2=prod_id2,shop=request.user,product_name=prod_name,prod_seller=prod_seller, category=category.objects.get(id=int(cat)),subcategory=subcategory,price=price,desc=desc,gst=gst_l,image1=image1).save()
