...
def perform_search(q):
    if q == "" or q == 'all':
        ...
        latest_skills = saler_models.Skill.objects.values(
            'tutor',).annotate(latest_id=Max('id'))
        unique_skills = saler_models.Skill.objects.filter(
            id__in=latest_skills.values('latest_id'))
    ...
    searched = saler_models.Skill.objects.filter(Q(title__icontains=q) | Q(
        tutor__username__icontains=q) | Q(tutor__first_name__icontains=q) | Q(tutor__last_name__icontains=q))
    # filterd=[]
